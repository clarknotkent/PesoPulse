<template>
  <div class="min-h-[100dvh] bg-[var(--bg)] flex items-center justify-center px-4 relative overflow-hidden">
    ...
    <div class="w-full max-w-sm relative space-y-6">
      <div class="space-y-3">
        <Logo />
        <p class="text-[var(--text-muted)] text-sm">Verify your email</p>
      </div>
      ...
      <div class="bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl p-5 space-y-4">
        <div class="flex items-start gap-3">
          <div class="w-10 h-10 rounded-full bg-emerald-500/15 flex items-center justify-center shrink-0">
            <Icon name="receipt" :size="18" class="text-emerald-400" />
          </div>
          <div class="space-y-1">
            <p class="text-[var(--text)] text-sm font-medium">Check your inbox</p>
        ...
        <p v-if="status" class="text-[10px] text-[var(--text-subtle)]">{{ status }}</p>
        ...
        <div class="flex gap-2">
          ...
          >
            ...
            <template v-else>{{ resending ? 'Sending…' : 'Resend email' }}</template>
          </button>
          <button
            @click="onCheck"
            :disabled="checking"
          ...
          >
            <Icon v-if="checking" name="loader" :size="14" class="animate-spin inline-block mr-1" />
            I've verified
          </button>
        </div>
...
if (process.client && emailVerified.value) {
