<template>
  <div class="page">
    <header class="page-header space-y-5">
      <h1 class="text-[var(--text)] font-medium">Budgets</h1>
      ...
      <div v-if="subTab === 'budgets'" class="space-y-3">
        <!-- Period toggle -->
        <div class="flex gap-1 bg-[var(--bg-input)] rounded-xl p-1">
          <button
        ...
        <div class="flex items-center justify-between gap-2">
          ...
          <p class="text-[var(--text)] text-sm font-medium tabular-nums">{{ anchorLabel }}</p>
          <button
            @click="shiftAnchor(1)"
            class="press w-10 h-10 rounded-lg border border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--text)] flex items-center justify-center"
            aria-label="Next"
    ...
    <div v-if="subTab === 'budgets'" class="page-body">
      <Transition name="swap" mode="out-in">
        <BudgetsSkeleton v-if="initialLoading" key="skeleton" />

        <div v-else key="content">
          <!-- Burn-down (earned card surface) -->
          <section class="page-section">
            <PeriodBurndown :period="period" :anchor="anchor" :key="period + anchor" />
          </section>
          ...
          <section class="page-section">
            ...
            <div v-if="editingTotal" class="flex gap-2">
              <input
                v-model="totalDraft"
                type="number"
                min="0"
              ...
              />
              <button @click="saveTotal" class="press bg-[var(--text)] text-[var(--bg)] font-medium px-4 rounded-lg text-sm">Save</button>
              <button
            ...
            <div v-else>
              <div class="flex items-baseline justify-between border-t border-[var(--border)] pt-4">
                <p class="text-3xl font-semibold text-[var(--text)] tabular-nums tracking-tightest">{{ formatPHP(totalSpent) }}</p>
                <p class="text-xs text-[var(--text-subtle)] tabular-nums">of {{ formatPHP(totalLimit + totalRollover) }}</p>
              </div>
              ...
              <p v-if="totalRollover > 0" class="text-[var(--c-income)] text-xs mt-2 tabular-nums">
                +{{ formatPHP(totalRollover) }} rollover from prior {{ rolloverNoun }}
              </p>
              <p v-if="data?.total.overspent" class="text-[var(--c-expense)] text-xs mt-2 tabular-nums">
          ...
          <section class="page-section">
            <p class="label mb-2">Per category</p>

            <div v-if="(data?.categories?.length ?? 0) > 0" class="border-t border-[var(--border)]">
              <BudgetRow
            ...
            <p v-else class="text-[var(--text-subtle)] text-sm py-6">No category budgets yet.</p>
          </section>
          ...
          <section class="page-section">
            <div class="bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl p-4 space-y-4">
              <p class="label">Set category budget</p>
          ...
          <p v-if="error" class="text-[var(--c-expense)] text-xs text-center py-2">{{ error }}</p>
        </div>
      </Transition>
    </div>
    ...
    <div v-else class="page-body">
      <NuxtLink to="/goals" class="press block bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl p-5 flex items-center justify-between">
        <div>
