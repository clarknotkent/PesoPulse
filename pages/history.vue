        ...
        >
          ...
          <Transition name="day">
            <div
              v-if="selectedDate"
              :key="selectedDate"
              class="bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl p-4 space-y-3"
            >
              <div class="flex items-center justify-between">
                ...
                <div class="text-[10px] tabular-nums text-[var(--text-subtle)] flex gap-2">
                  <span
                    v-if="dayTotals.income > 0"
                    class="text-[var(--c-income)]"
                  >+{{ formatPHP(dayTotals.income) }}</span>
                  <span
                    v-if="dayTotals.expense > 0"
                    class="text-[var(--c-expense)]"
                  >-{{ formatPHP(dayTotals.expense) }}</span>
                </div>
              </div>

              <div
                v-if="dayTxns.length === 0"
                class="text-[var(--text-subtle)] text-xs text-center py-4"
              >
        ...
        >
          ...
          >
            <div class="flex items-center justify-between px-1">
              <p class="text-[var(--text-subtle)] text-[10px] uppercase tracking-widest">
                {{ group.label }}
              </p>
              <p class="text-[10px] tabular-nums text-[var(--text-subtle)]">
                <span
                  v-if="group.income > 0"
                  class="text-[var(--c-income)]"
                >+{{ formatPHP(group.income) }}</span>
                <span v-if="group.income > 0 && group.expense > 0"> · </span>
                <span
                  v-if="group.expense > 0"
                  class="text-[var(--c-expense)]"
                >-{{ formatPHP(group.expense) }}</span>
              </p>
            </div>
            <TransactionListItem
