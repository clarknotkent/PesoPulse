  ...
  <div class="goal-row py-5 border-b border-[var(--border)] flex items-center gap-4">
    ...
    <div class="relative w-16 h-16 shrink-0">
      ...
      <div class="absolute inset-0 flex items-center justify-center">
        <p class="text-[var(--text)] text-[11px] font-semibold tabular-nums">
          {{ Math.round(goal.pctComplete) }}%
        </p>
      </div>
    </div>
    ...
    <div class="min-w-0 flex-1">
      ...
      <p class="text-[var(--text-subtle)] text-xs mt-0.5 tabular-nums">
        {{ goal.category }} · {{ goal.daysLeft }}d left
      </p>
