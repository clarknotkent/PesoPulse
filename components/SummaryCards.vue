<template>
  <div class="grid grid-cols-1 sm:grid-cols-[1.4fr_1fr] gap-6 items-stretch">
    ...
    <div class="py-1">
      ...
      >{{ formatPHP(net) }}</p>
      <p v-if="deltaNet !== undefined" class="text-xs mt-2 tabular-nums" :class="deltaNet >= 0 ? 'text-[var(--c-income)]' : 'text-[var(--c-expense)]'">
    ...
    <div class="divide-y divide-[var(--border)] border-y border-[var(--border)]">
      <div class="flex items-baseline justify-between py-3">
        ...
        <div class="text-right">
          <p class="text-[var(--c-income)] font-semibold tabular-nums">{{ formatPHP(income) }}</p>
          <p v-if="deltaIncome !== undefined" class="text-[10px] tabular-nums" :class="deltaIncome >= 0 ? 'text-[var(--c-income)]' : 'text-[var(--c-expense)]'">
      ...
      <div class="flex items-baseline justify-between py-3">
        ...
        <div class="text-right">
          <p class="text-[var(--c-expense)] font-semibold tabular-nums">{{ formatPHP(expense) }}</p>
          <p v-if="deltaExpense !== undefined" class="text-[10px] tabular-nums" :class="deltaExpense <= 0 ? 'text-[var(--c-income)]' : 'text-[var(--c-expense)]'">
      ...
      <div class="flex items-baseline justify-between py-3">
        ...
        <p class="text-[var(--text)] font-semibold tabular-nums">{{ savingsRate.toFixed(1) }}%</p>
      </div>
    </div>
  </div>
</template>
